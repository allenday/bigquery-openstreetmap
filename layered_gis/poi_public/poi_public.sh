...
do
  ...
  echo "SELECT
  $CODE AS layer_code, 'poi_public' AS layer_class, '$V' AS layer_name, feature_type AS gdal_type, osm_id, osm_version, osm_timestamp, all_tags, geometry
...
#2008
echo "SELECT
  2008 AS layer_code, 'poi_public' AS layer_class, 'town_hall' AS layer_name, feature_type AS gdal_type, osm_id, osm_version, osm_timestamp, all_tags, geometry
FROM \`${GCP_PROJECT}.${BQ_DATASET}.features\`
WHERE
...
#2030
echo "SELECT
  2030 AS layer_code, 'poi_public' AS layer_class, 'recycling' AS layer_name, feature_type AS gdal_type, osm_id, osm_version, osm_timestamp, all_tags, geometry
FROM \`${GCP_PROJECT}.${BQ_DATASET}.features\`
WHERE
...
#2031
echo "SELECT
  2031 AS layer_code, 'poi_public' AS layer_class, 'recycling_glass' AS layer_name, feature_type AS gdal_type, osm_id, osm_version, osm_timestamp, all_tags, geometry
FROM \`${GCP_PROJECT}.${BQ_DATASET}.features\`
WHERE
