...
do
  ...
  V="${KV##*=}"
  echo "SELECT
  $CODE AS layer_code, 'natural' AS layer_class, '$V' AS layer_name, feature_type AS gdal_type, osm_id, osm_version, osm_timestamp, all_tags, geometry
...
#4131 
  echo "SELECT
  4131 AS layer_code, 'natural' AS layer_class, 'mine' AS layer_name, feature_type AS gdal_type, osm_id, osm_version, osm_timestamp, all_tags, geometry
FROM \`${GCP_PROJECT}.${BQ_DATASET}.features\`
