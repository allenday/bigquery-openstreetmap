...
do
  ...
  V="${KV##*=}"
  echo "SELECT
  $CODE AS layer_code, 'barriers' AS layer_class, '$V' AS layer_name, feature_type AS gdal_type, osm_id, osm_way_id, osm_timestamp, all_tags, geometry
...
echo "SELECT
  5501 AS layer_code, 'barriers' AS layer_class, 'fence' AS layer_name, feature_type AS gdal_type, osm_id, osm_way_id, osm_timestamp, all_tags, geometry
FROM \`${GCP_PROJECT}.${BQ_DATASET}.features\`
