...
SELECT
  5311 AS layer_code, 'traffic' AS layer_class, 'dam' AS layer_name, f.feature_type AS gdal_type, f.osm_id, f.osm_way_id, f.osm_timestamp, osm.all_tags, f.geometry
FROM
  `openstreetmap-public-data-dev.osm_planet.features` AS f, osm
WHERE EXISTS(SELECT 1 FROM UNNEST(osm.all_tags) as tags WHERE tags.key = 'waterway' AND tags.value='dam') AND osm.id = f.osm_id
UNION ALL
SELECT
  5311 AS layer_code, 'traffic' AS layer_class, 'dam' AS layer_name, f.feature_type AS gdal_type, f.osm_id, f.osm_way_id, f.osm_timestamp, osm.all_tags, f.geometry
FROM
  `openstreetmap-public-data-dev.osm_planet.features` AS f, osm
WHERE EXISTS(SELECT 1 FROM UNNEST(osm.all_tags) as tags WHERE tags.key = 'waterway' AND tags.value='dam') AND osm.way_id = f.osm_way_id
