...
do
  ...
  V="${KV##*=}"
  echo "SELECT
  $CODE AS layer_code, 'place' AS layer_class, '$V' AS layer_name, feature_type AS gdal_type, osm_id, osm_version, osm_timestamp, all_tags, geometry
...
#1005
echo "SELECT
  1005 AS layer_code, 'place' AS layer_class, 'national_capital' AS layer_name, feature_type AS gdal_type, osm_id, osm_version, osm_timestamp, all_tags, geometry
FROM \`${GCP_PROJECT}.${BQ_DATASET}.features\`
WHERE (
...
#1031
echo "SELECT
  1031 AS layer_code, 'place' AS layer_class, 'dwelling' AS layer_name, feature_type AS gdal_type, osm_id, osm_version, osm_timestamp, all_tags, geometry
FROM \`${GCP_PROJECT}.${BQ_DATASET}.features\`
...
#1099
echo "SELECT
  1099 AS layer_code, 'place' AS layer_class, 'named_place' AS layer_name, feature_type AS gdal_type, osm_id, osm_version, osm_timestamp, all_tags, geometry
FROM \`${GCP_PROJECT}.${BQ_DATASET}.features\`
WHERE
