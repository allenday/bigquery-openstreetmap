...
do
  ...
  V="${KV##*=}"
  echo "SELECT
  $CODE AS layer_code, 'land_use' AS layer_class, '$V' AS layer_name, feature_type AS gdal_type, osm_id, osm_version, osm_timestamp, all_tags, geometry 
...
#7201
echo "SELECT
  7201 AS layer_code, 'land_use' AS layer_class, 'forest' AS layer_name, feature_type AS gdal_type, osm_id, osm_version, osm_timestamp, all_tags, geometry 
FROM \`${GCP_PROJECT}.${BQ_DATASET}.features\`
...
#7202
echo "SELECT
  7202 AS layer_code, 'land_use' AS layer_class, 'park' AS layer_name, feature_type AS gdal_type, osm_id, osm_version, osm_timestamp, all_tags, geometry 
FROM \`${GCP_PROJECT}.${BQ_DATASET}.features\`
