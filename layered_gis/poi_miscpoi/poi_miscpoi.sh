...
do
  ...
  V="${KV##*=}"
  echo "SELECT
  $CODE AS layer_code, 'poi_miscpoi' AS layer_class, '$V' AS layer_name, feature_type AS gdal_type, osm_id, osm_timestamp, all_tags, geometry
...
#2901
echo "SELECT
  2901 AS layer_code, 'poi_miscpoi' AS layer_class, 'toilet' AS layer_name, feature_type AS gdal_type, osm_id, osm_timestamp, all_tags, geometry
FROM \`${GCP_PROJECT}.${BQ_SOURCE_DATASET}.features\`
WHERE EXISTS(SELECT 1 FROM UNNEST(all_tags) as tags WHERE tags.key = 'amenity' AND tags.value='toilets')" > "toilet.sql"

#2907
echo "SELECT
  2907 AS layer_code, 'poi_miscpoi' AS layer_class, 'camera_surveillance' AS layer_name, feature_type AS gdal_type, osm_id, osm_timestamp, all_tags, geometry
FROM \`${GCP_PROJECT}.${BQ_SOURCE_DATASET}.features\`
...
#2921
echo "SELECT
  2921 AS layer_code, 'poi_miscpoi' AS layer_class, 'emergency_phone' AS layer_name, feature_type AS gdal_type, osm_id, osm_timestamp, all_tags, geometry
FROM \`${GCP_PROJECT}.${BQ_SOURCE_DATASET}.features\`
...
#2922
echo "SELECT
  2922 AS layer_code, 'poi_miscpoi' AS layer_class, 'fire_hydrant' AS layer_name, feature_type AS gdal_type, osm_id, osm_timestamp, all_tags, geometry
FROM \`${GCP_PROJECT}.${BQ_SOURCE_DATASET}.features\`
...
#2923
echo "SELECT
  2923 AS layer_code, 'poi_miscpoi' AS layer_class, 'emergency_access' AS layer_name, feature_type AS gdal_type, osm_id, osm_timestamp, all_tags, geometry
FROM \`${GCP_PROJECT}.${BQ_SOURCE_DATASET}.features\`
...
#2950
echo "SELECT
  2950 AS layer_code, 'poi_miscpoi' AS layer_class, 'tower' AS layer_name, feature_type AS gdal_type, osm_id, osm_timestamp, all_tags, geometry
FROM \`${GCP_PROJECT}.${BQ_SOURCE_DATASET}.features\`
WHERE EXISTS(SELECT 1 FROM UNNEST(all_tags) as tags WHERE tags.key = 'man_made' AND tags.value='tower')
  ...
  AND NOT EXISTS(SELECT 1 FROM UNNEST(all_tags) as tags WHERE tags.key = 'tower:type' AND tags.value='observation')" > "tower.sql"

#2951
echo "SELECT
  2951 AS layer_code, 'poi_miscpoi' AS layer_class, 'tower_comms' AS layer_name, feature_type AS gdal_type, osm_id, osm_timestamp, all_tags, geometry
FROM \`${GCP_PROJECT}.${BQ_SOURCE_DATASET}.features\`
WHERE EXISTS(SELECT 1 FROM UNNEST(all_tags) as tags WHERE tags.key = 'man_made' AND tags.value='tower')
...
#2953
echo "SELECT
  2953 AS layer_code, 'poi_miscpoi' AS layer_class, 'tower_observation' AS layer_name, feature_type AS gdal_type, osm_id, osm_timestamp, all_tags, geometry
FROM \`${GCP_PROJECT}.${BQ_SOURCE_DATASET}.features\`
WHERE EXISTS(SELECT 1 FROM UNNEST(all_tags) as tags WHERE tags.key = 'man_made' AND tags.value='tower')
...
#2963
echo "SELECT
  2963 AS layer_code, 'poi_miscpoi' AS layer_class, 'water_mill' AS layer_name, feature_type AS gdal_type, osm_id, osm_timestamp, all_tags, geometry
FROM \`${GCP_PROJECT}.${BQ_SOURCE_DATASET}.features\`
