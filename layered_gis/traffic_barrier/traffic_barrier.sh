...
do
  ...
  V="${KV##*=}"
  echo "SELECT
  $CODE AS layer_code, 'traffic' AS layer_class, 'barrier_$V' AS layer_name, feature_type AS gdal_type, osm_id, osm_version, osm_timestamp, all_tags, geometry
...
#5210
echo "SELECT
  5210 AS layer_code, 'traffic' AS layer_class, 'barrier' AS layer_name, feature_type AS gdal_type, osm_id, osm_version, osm_timestamp, all_tags, geometry
FROM \`${GCP_PROJECT}.${BQ_DATASET}.features\`
...
#5214
echo "SELECT
  5214 AS layer_code, 'barrier' AS layer_class, 'stile' AS layer_name, feature_type AS gdal_type, osm_id, osm_version, osm_timestamp, all_tags, geometry
FROM \`${GCP_PROJECT}.${BQ_DATASET}.features\`
WHERE EXISTS(SELECT 1 FROM UNNEST(all_tags) as tags WHERE tags.key IN ('barrier','highway') AND tags.value='stile')" > "stile.sql"
#5215
echo "SELECT
  5217 AS layer_code, 'barrier' AS layer_class, 'cycle' AS layer_name, feature_type AS gdal_type, osm_id, osm_version, osm_timestamp, all_tags, geometry
FROM \`${GCP_PROJECT}.${BQ_DATASET}.features\`
WHERE EXISTS(SELECT 1 FROM UNNEST(all_tags) as tags WHERE tags.key = 'barrier' AND tags.value='cycle_barrier')" > "cycle.sql"
#5217
echo "SELECT
  5217 AS layer_code, 'barrier' AS layer_class, 'toll' AS layer_name, feature_type AS gdal_type, osm_id, osm_version, osm_timestamp, all_tags, geometry
FROM \`${GCP_PROJECT}.${BQ_DATASET}.features\`
