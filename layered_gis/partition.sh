...
echo "CREATE OR REPLACE FUNCTION \`${GCP_PROJECT}.${BQ_DATASET}.gd\`(j INT64)
RETURNS ARRAY<DATE>
...
echo "CREATE OR REPLACE FUNCTION \`${GCP_PROJECT}.${BQ_DATASET}.layer_partition\`(c STRING)
RETURNS ARRAY<DATE>
AS(CASE"
N=1
for LAYER in $LAYERS
do
  ...
  do
    ...
    echo "WHEN c='$LAYER-$LAYER_CLASS' THEN \`${GCP_PROJECT}.${BQ_DATASET}.gd\`($N)"
    K1=$N
    N=$((N+1))
  done
  echo "WHEN c='$LAYER' THEN \`${GCP_PROJECT}.${BQ_DATASET}.gda\`($K0,$K1)"
done
