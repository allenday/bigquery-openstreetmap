...
def get_queries() -> List[str]:
    """gets SQL query files from Cloud Storage bucket. It expects them to be in "folder" bq_maps
    ...
    blobs = bucket.list_blobs(prefix='bq_maps')
    queries = {}
    for blob in blobs:
        blob_name = blob.name
        if '.sh' in blob_name:
            continue
        filename = blob_name.replace('bq_maps/', '')
